...
async def log_request(request: httpx.Request):
    print(f"HTTPX request | {request.method} {request.url} | CONTENT: {request.content!r}")

async def log_response(response: httpx.Response):
    request = response.request
    print(f"HTTPX response | {request.method} {request.url} | STATUS {response.status_code}")
